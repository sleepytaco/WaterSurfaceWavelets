...
// NOTE: interpolateAmplitude4d is rewritten in basis.cpp
...
double Amplitude::bilerp(Vector2d x, double theta, double waveNumber) {
    ...
    // Obtain relevant values adjacent to idxSpacePos
    double amp00 = m_currentAmplitude.get(i  , j  , theta, 0);
    double amp01 = m_currentAmplitude.get(i, j+1  , theta, 0);
    double amp10 = m_currentAmplitude.get(i+1  , j, theta, 0);
    double amp11 = m_currentAmplitude.get(i+1, j+1, theta, 0);

    //    std::cout << x.x() << "," << x.y() << std::endl;
    // Perform bilinear interpolation
    // Unit square leads to to following weighting: https://en.wikipedia.org/wiki/Bilinear_interpolation
    double interpolatedValue = amp00 * (1 - dx) * (1 - dy) +
                               amp01 * (1 - dx) * dy +
                               amp10 * dx * (1 - dy) +
                               amp11 * dx * dy;
